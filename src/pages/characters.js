...
import charactersAPI from "../util/ApiChar"
import ImgAvatar from "../components/ImgAvatar";

export default function Characters() {
  const [charactersInfo, setCharactersInfo] = useState(charactersAPI)
  ...
  function handel_N_Valid() {
    ...
    if (N_ChangeValue.trim() != "") {
      Characters.forEach((e) => {
        if (e.id == activeChar.id) return e.name.avatarName = N_ChangeValue
      })
      setN_Change(!N_Change)
      setN_ChangeValue("")
    }
    ...
    <div className="content">
      ...
      <div className="avatar-info">
        ...
        <div className="info">
          ...
          <Outlet context={{ activeChar, isChnaged: N_Change , N_ChangeValue , onHandel_N_Valid: handel_N_Valid , onHandel_N_Change: handel_N_Change , onHandel_N_Click: handel_N_Click }} />
        </div>
      </div>
    </div>
  )
